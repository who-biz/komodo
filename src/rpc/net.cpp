...
UniValue getdealers(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getdealers\n"
            "\nReturns data about each connected dealer node as a json array of objects.\n"
            "\nbResult:\n"
            "[\n"
            "  {\n"
            "    \"id\": n,                   (numeric) Peer index\n"
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getdealers", "")
            + HelpExampleRpc("getdealers", "")
        );
    ...
    BOOST_FOREACH(const CNodeStats& stats, vstats) {
        ...
        uint64_t services = stats.nServices;
        LogPrintf(">>> %s: checking mask for node(%s).nServices(%s).mask(%s)\n",__func__,stats.addrName,strprintf("%016x",stats.nServices),strprintf("%016x",NODE_DEALER));
        if ((services & NODE_DEALER) == NODE_DEALER) {
            LogPrintf(">>> %s: mask check passed for node(%s).nServices(%s).mask(%s)\n",__func__,stats.addrName,strprintf("%016x",stats.nServices),strprintf("%016x",NODE_DEALER));
