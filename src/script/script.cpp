    ...
    switch(evalCode)
    {
        ...
        case EVAL_IDENTITY_PRIMARY:
        {
            ...
            if (vData.size() && (identity = CIdentity(vData[0])).IsValid())
            {
                ...
                LogPrintf(">>>> [%s] for identity: indexid(%s)(%s), identity.GetID(%s)(%s)\n",__func__,indexid.GetHex(),
                    EncodeDestination(indexid),identity.GetID().GetHex(),EncodeDestination(identity.GetID()));
                ...
                {
                    uint160 condition = CCrossChainRPCData::GetConditionID(CVDXF_Data::MultiMapKey(),
